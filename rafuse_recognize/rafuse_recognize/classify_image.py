...
class NodeLookup(object):
  """Converts integer node ID's to human readable labels. 将整数节点ID转换为人类可读的标签。"""
  ...
  def load(self, label_lookup_path, uid_lookup_path):
    ...
    # Loads mapping from string UID to human-readable string #加载从字符串UID到人类可读字符串
    proto_as_ascii_lines = tf.gfile.GFile(uid_lookup_path).readlines()#tf.gfile.GFile类似于python提供的文本操作open()函数
    uid_to_human = {}
    #p = re.compile(r'[n\d]*[ \S,]*')   #利用正则表达式提取需要的信息
    ...
    # Loads mapping from string UID to integer node ID.#加载从字符串UID到整数节点ID的映射
    node_id_to_uid = {}
    ...
    # Loads the final mapping of integer node ID to human-readable string  #加载整数节点ID到人类可读字符串的最终映射
    node_id_to_name = {}
  ...
  def load_chinese_map(self, uid_chinese_lookup_path):#将UID从字符串加载到可读字符串
...
if __name__ == '__main__':
  ...
  parser.add_argument(  #这个函数下载数据集用的
      '--model_dir',
      type=str,
      default='C:/Users/zhengliangjin/Desktop/data',
      help="""\
  ...
  parser.add_argument(  #这个时图片文件的文件夹存放位置
      '--image_file',
      type=str,
      default='',
      help='Absolute path to image file.'
  )
  parser.add_argument(   #这个函数顶预测用的
      '--num_top_predictions',
      type=int,
      default=5,
