...
class TextCNN(object):
    ...
    def __init__(
      ...
      embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0):
        ...
        # 输入、输出和退出的占位符
        # tf.placeholder定义一个变量和维度，tf.int32和tf.float32指变量类型，None不限维度，
        ...
        # Add dropout       # Add dropout 添加dropout
        with tf.name_scope("dropout"):
            self.h_drop = tf.nn.dropout(self.h_pool_flat, self.dropout_keep_prob)

        # Final (unnormalized) scores and predictions# Final (unnormalized) scores and predictions
